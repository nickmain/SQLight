...
public extension SQLight.PreparedStatement {
    ...
    var parameterCount: Int { Int(sqlite3_bind_parameter_count(statementPtr)) }
    ...
    func indexOfParameter(named name: String) -> Int? {
        let index = Int(sqlite3_bind_parameter_index(statementPtr, name))
    ...
    func nameOfParameter(at index: Int) -> String? {
        let ptr = sqlite3_bind_parameter_name(statementPtr, Int32(index))
    ...
    func bindParameterToNull(at index: Int) throws {
        let rc = sqlite3_bind_null(statementPtr, Int32(index))
    ...
    func bindParameter(at index: Int, to value: Int) throws {
        let rc = sqlite3_bind_int64(statementPtr, Int32(index), Int64(value))
        guard rc == SQLITE_OK else {
            throw SQLight.Error.result(.fromSQLite(code: rc))
        }
    }
    ...
    func bindParameter(at index: Int, to value: Double) throws {
        let rc = sqlite3_bind_double(statementPtr, Int32(index), value)
        guard rc == SQLITE_OK else {
            throw SQLight.Error.result(.fromSQLite(code: rc))
        }
    }
    ...
    func bindParameter(at index: Int, to value: String) throws {
        let rc = sqlite3_bind_text(statementPtr, Int32(index), value, -1, SQLight.Value.SQLITE_TRANSIENT)
        guard rc == SQLITE_OK else {
            throw SQLight.Error.result(.fromSQLite(code: rc))
        }
    }
    ...
    func bindParameter(at index: Int, to value: Data) throws {
        try value.withUnsafeBytes { (ptr: UnsafeRawBufferPointer) in
            let rc = sqlite3_bind_blob(statementPtr, Int32(index), ptr.baseAddress, Int32(ptr.count), SQLight.Value.SQLITE_TRANSIENT)
            guard rc == SQLITE_OK else {
                throw SQLight.Error.result(.fromSQLite(code: rc))
            }
        }
    ...
    func bindParameter(at index: Int, toZeroBlobOfSize size: Int) throws {
        let rc = sqlite3_bind_zeroblob(statementPtr, Int32(index), Int32(size))
        guard rc == SQLITE_OK else {
            throw SQLight.Error.result(.fromSQLite(code: rc))
        }
    }
    ...
    func clearBindings() {
        sqlite3_clear_bindings(statementPtr)
    }
}
