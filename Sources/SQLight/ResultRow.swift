...
public extension SQLight.PreparedStatement {
    ...
    var rowColumnCount: Int {
        Int(sqlite3_data_count(statementPtr))
    }
    ...
    func columnType(at index: Int) -> SQLight.ValueType {
        .init(rawValue: Int(sqlite3_column_type(statementPtr, Int32(index)))) ?? .null
    }

    /// Whether a column is null
    func isColumnNull(at index: Int) -> Bool {
        sqlite3_column_type(statementPtr, Int32(index)) == SQLight.ValueType.null.rawValue
    }

    /// Get the value of a column as a Double
    func doubleValue(at index: Int) -> Double {
        sqlite3_column_double(statementPtr, Int32(index))
    }

    /// Get the value of a column as an Int
    func integerValue(at index: Int) -> Int {
        Int(sqlite3_column_int64(statementPtr, Int32(index)))
    }
    ...
    func stringValue(at index: Int) -> String? {
        let ptr = sqlite3_column_text(statementPtr, Int32(index))
    ...
    func dataValue(at index: Int) -> Data? {
        let ptr = sqlite3_column_blob(statementPtr, Int32(index))
        guard let ptr else { return nil }

        let size = sqlite3_column_bytes(statementPtr, Int32(index))
